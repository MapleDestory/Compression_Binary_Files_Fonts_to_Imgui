...
int main(int argc, char** argv)
{
	...
	char symb_name[1024] = { 0 };

	memset(&file_name, 0, 1024);
	memset(&symb_name, 0, 1024);
	...
	for (int i = 0; i < argc; i++)
	{
		...
		if (_stricmp(argv[i], "/f") == 0)
			strcpy_s(file_name, argv[i + 1]);

		if (_stricmp(argv[i], "/s") == 0)
			strcpy_s(symb_name, argv[i + 1]);
	}
	
	
	binary_to_compressed_c(file_name, symb_name, use_base85_encoding, use_compression);
...
bool binary_to_compressed_c(const char* filename, const char* symbol, bool use_base85_encoding, bool use_compression)
{
	...
	std::string type = input.substr(input.find_last_of("."));
	std::string filename_s = input.replace(input.find_last_of("."), type.length(), ".h");
	std::string defined = input.substr(input.find_last_of("\\") + 1);
	defined = defined.replace(defined.find_last_of("."), type.length(), "");
	...
	{
		fclose(f); delete[] data; return false;
	}
	...
	FILE* out = fopen(filename_s.c_str(), "w+");
	...
	{
		fprintf(out, "static const char %s_%sdata_base85[%d+1] =\n    \"", filename_s.c_str(), compressed_str, (int)((compressed_sz + 3) / 4) * 5);
		char prev_c = 0;
	...
	{
		fprintf(out, "static const unsigned int %s_%ssize = %d;\n", defined.c_str(), compressed_str, (int)compressed_sz);
		fprintf(out, "static const unsigned int %s_%sdata[%d/4] =\n{", defined.c_str(), compressed_str, (int)((compressed_sz + 3) / 4) * 4);
		int column = 0;
...
{
	stb_out(v >> 16); stb_out(v >> 8); stb_out(v);
}
static void stb_out4(stb_uint v)
{
	stb_out(v >> 24); stb_out(v >> 16); stb_out(v >> 8); stb_out(v);
}
